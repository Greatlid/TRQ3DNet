...
def train_options(parser):
    ...
    parser.add_argument('--arch', '-a', metavar='ARCH', required=False, choices=model_names,
                        help = 'model architecture: ' + ' | '.join(model_names), default='trq3dv0' )
    ...
    parser.add_argument('--dataroot', '-d', type=str,
                        default='./data/ICVL64_31.npz', help='data root')
...
class Engine(object):
    ...
    def load(self, resumePath=None, load_opt=True):
        ...
        try:
            self.writerlogs.logs.update(checkpoint['logs'])
        except: pass
    def get_net(self):
    ...
    def __step(self, train, inputs, targets):
        ...
        if self.get_net().bandwise:
            ...
            time_end = time.time()
            # eval_time.append(time_end - time_start)
            # print(eval_time)
            # if len(eval_time) >= 1:
            #     print(np.mean(eval_time[1:]))
            # print('eval time: ', time_end - time_start)
            if isinstance(outputs, tuple):
                # loss_0 = self.criterion(outputs[0], targets)
                # loss_1 = self.criterion(outputs[1], targets)
                # loss = loss_0 + loss_1
    ...
    def train(self, train_loader):
        ...
        for batch_idx, (inputs, targets) in enumerate(train_loader):
            ...
            if not self.opt.no_log:
                self.writerlogs.add_scalar('train_loss', (loss_data, self.iteration))
                self.writerlogs.add_scalar('train_avg_loss',(avg_loss, self.iteration))
        ...
        self.epoch += 1
        if not self.opt.no_log:
            self.writerlogs.add_scalar('train_loss_epoch', (avg_loss, self.epoch))
            self.writer.add_scalar(
    ...
    def validate(self, valid_loader, name):
        ...
        if not self.opt.no_log:
            self.writerlogs.add_scalar('val_loss_epoch', (avg_loss, self.epoch))
            self.writerlogs.add_scalar('val_psnr_epoch', (avg_psnr, self.epoch))
            self.writer.add_scalar(
